...
impl Theme {
	...
	pub fn ema() -> Theme {
		Theme {
			...
			highlighted_foreground: Color::Rgb {
				r: 0x43,
				g: 0x26,
				b: 0x16,
			},
			highlighted_background: Color::Rgb {
				r: 0x9a,
				g: 0x78,
				b: 0x4f,
			},
...
impl SelectableText {
	...
	pub fn get_styled_content(&self) -> StyledContent<String> {
		match self.is_highlighted {
			true => StyledContent::new(self.current_theme.get_highlight_style(), self.text.clone()),
			false => StyledContent::new(self.current_theme.get_content_style(), self.text.clone()),
		}
	}
}
