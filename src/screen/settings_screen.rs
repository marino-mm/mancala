...
use crate::theme::{SelectableText, Theme, color_to_rgb};
...
impl ThemeListWindow {
	fn new() -> ThemeListWindow {
		...
		bindings.insert(
			...
			ThemeListWindow::move_theme_list_index_up,
		);
		bindings.insert(
			KeyEvent::new(KeyCode::Down, KeyModifiers::NONE),
			ThemeListWindow::move_theme_list_index_down,
		);
	...
	fn print_window(&self, app: &App) {
		...
		for n in 0..self.window_height {
			let formated_line = match self.theme_list.get(n as usize) {
				...
				None => {
					format!("║{: <width$}║", " ".to_string(), width = content_width)
				}
			};
...
impl ThemeDetailWindow {
	...
	fn print_window(&self, app: &App, state: &Settings) {
		...
		let display_variable_iterator = display_variable_list
			...
			.enumerate()
			.map(|(i, c)| {
				SelectableText::new(c.clone(), i == self.selected_index as usize, current_theme.clone())
					.get_styled_content()
			})
