...
impl Game {
	...
	fn is_free_move(&self, last_pit: u8) -> bool {
		(self.current_player_id == self.player1.id && self.get_player_score_pit_position(self.player1.id) == last_pit)
			|| (self.current_player_id == self.player2.id
	...
	fn print_board(&self) {
		...
		for line in lines {
			queue!(stdout, Clear(CurrentLine), Print(line), cursor::MoveToNextLine(1)).unwrap();
		}
